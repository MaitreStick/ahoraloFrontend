...
export const BasketScreenTab = () => {
  ...
  const handleViewRoute = async () => {
    ...
    const companyIds = Array.from(new Set(items.map(item => item.comcity.company.id))).filter(Boolean);
    ...
    try {
      ...
      const nearestWarehouses = companyIds.map(companyId => {
        const companyWarehouses = warehousesData.filter(warehouse => warehouse.companyId === companyId);

        if (companyWarehouses.length === 0) return null;
      ...
      if (nearestWarehouses.length === 0) {
        console.warn('No se encontraron almacenes cercanos para las empresas en la canasta.');
        return;
      }

      nearestWarehouses.sort((a, b) => (a?.distance ?? Infinity) - (b?.distance ?? Infinity));

      const locations = [
        `${lastKnownLocation.latitude},${lastKnownLocation.longitude}`,
        ...nearestWarehouses.map(loc => loc ? `${loc.latitude},${loc.longitude}` : '').filter(Boolean),
      ];

      let url = `https://www.google.com/maps/dir/?api=1&origin=${locations[0]}&destination=${locations[locations.length - 1]}`;
