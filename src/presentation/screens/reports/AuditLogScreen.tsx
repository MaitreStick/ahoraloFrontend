...
import { View, Text, TouchableOpacity, FlatList, ScrollView, Modal, TouchableWithoutFeedback, StyleSheet, Alert } from 'react-native';
...
export const AuditLogsScreen = () => {
    ...
    const [selectedUserId, setSelectedUserId] = useState('Seleccionar ID de Usuario');
    ...
    const handleActionSelect = (action: string) => {
        setFilters({ ...filters, action: action === 'Todas las acciones' ? '' : action });
    ...
    const renderItem = ({ item }: { item: AuditLog }) => (
        <View style={styles.row}>
            ...
            <Text style={styles.cell}>{item.old_value ? JSON.stringify(item.old_value) : 'N/A'}</Text>
            <Text style={styles.cell}>{item.new_value ? JSON.stringify(item.new_value) : 'N/A'}</Text>
            ...
            <ScrollView horizontal>
                <View>
                    ...
                    <FlatList
                        ...
                        onRefresh={loadLogs}
                        refreshing={loading}
                        ListEmptyComponent={<Text style={styles.emptyText}>No se encontraron registros de auditoría</Text>}
                    />
                </View>
            </ScrollView>
            ...
            >
                ...
                <View style={styles.modalContentContainer}>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <TouchableOpacity onPress={() => handleUserSelect(item)} style={styles.modalItem}>
            ...
            >
                ...
                <View style={styles.modalContentContainer}>
                    ...
                    {['Todas las acciones', 'create', 'update', 'delete'].map((action) => (
                        <TouchableOpacity key={action} onPress={() => handleActionSelect(action)} style={styles.modalItem}>
                            <Text style={styles.modalText}>{action === 'Todas las acciones' ? action : action.charAt(0).toUpperCase() + action.slice(1)}</Text>
                        </TouchableOpacity>
                    ))}
                </View>
            </Modal>
