...
export const updateCreateProdcomcity = (prodcomcity: Partial<Prodcomcity>) => {
  prodcomcity.price = isNaN(Number(prodcomcity.price))
    ? 0
    : Number(prodcomcity.price);
  if (prodcomcity.product) {
    prodcomcity.product.code = isNaN(Number(prodcomcity.product.code))
      ? 0
      : Number(prodcomcity.product.code);
  }

  if (
    prodcomcity.id &&
    prodcomcity.comcity?.id !== 'new' &&
    prodcomcity.product?.id !== 'new'
  ) {
