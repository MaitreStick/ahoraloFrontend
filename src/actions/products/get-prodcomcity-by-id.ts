...
export const getProdComCityByIds = async (
  comcityId: string,
  productId: string,
): Promise<Prodcomcity> => {
  if (comcityId === 'new' && productId === 'new') {
    return emptyProdcomcity;
  }

  try {
    const {data} = await ahoraloApi.get<AhoraloProdcomcity>(
      `/prodcomcity/${comcityId}/${productId}`,
    );
    ...
    console.error(error);
    throw new Error(
      `Error al obtener el prodcomcity con comcityId ${comcityId} y productId ${productId}`,
    );
  }
};
